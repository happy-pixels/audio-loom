...
export class StereoPanner {
    private distanceCallbacks: Map<string, {
        config: DistanceCallbackConfig;
        lastDistances: Map<number, boolean>;
    ...
    play2DPanned(key: string, options?: Play2DPannedOptions): string | null {
        ...
        const instance: PannedSoundInstance = {
            id: instanceId,
            key,
            group,
            sourceNode,
    ...
    registerDistanceCallback(instanceId: string, config: DistanceCallbackConfig): boolean {
        ...
        let found = false;
        let soundPosition: Vector3 | null = null;
        ...
        const lastDistances = new Map<number, boolean>();
    ...
    private checkDistanceThresholds(instanceId: string, config: DistanceCallbackConfig): void {
        const entry = this.distanceCallbacks.get(instanceId);
        if (!entry) return;
        ...
        for (const threshold of config.thresholds) {
            ...
            if (wasInside !== isInside) {
                const direction: 'entering' | 'leaving' = isInside ? 'entering' : 'leaving';
